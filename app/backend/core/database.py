...
class DatabaseManager:
    ...
    async def ensure_initialized(self):
        ...
        # We check _initialized to ensure both engine and tables are ready
        if self.async_session_maker is not None and self._initialized:
            return

        # init_db and create_tables each handle their own internal locking
        # which safely manages concurrent initialization attempts.
        try:
            logger.warning("Database not fully initialized, attempting lazy initialization...")
            await self.init_db()
